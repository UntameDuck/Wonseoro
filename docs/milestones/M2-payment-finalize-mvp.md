...
| 구분 | 태스크 |
|---|---|
| ✅ **M2 완료** | 백엔드 11개 + T-M1-09 + **프론트 13개 전부** |
| 🔜 다음 | **M3 운영 안전장치** |
| 🔜 프론트 | T-M2-20~32 (전체) |

### 📎 서류 파이프라인 화면 연결 (2026-09-23)

브라우저 → Object Storage 직접 업로드 → 검사 워커 → AVAILABLE 까지 화면에서 확인했다.

```
파일 선택 → "업로드를 준비하는 중…" → "업로드 중…" → "파일을 확인하는 중…"
         → "업로드 완료 — 악성코드 검사 중입니다"   (doc QUARANTINED / scan PENDING)
document-service 가 2초 주기로 가져가 검사 (Mock, 1.5초 지연)
         → "✓ 검사 완료"                          (doc AVAILABLE / scan CLEAN / mock-av)
5단계 검토: 서류 "검사 완료" · 입력항목 "누락 없음" · 전형료 60,000원
```

- 파일은 접수 API 를 지나지 않는다. Presigned URL 로 브라우저가 직접 올린다
- 브라우저가 계산한 sha256 을 서버가 **다시 계산해 대조**한다
- 워커는 DB 를 직접 고치지 않는다. 상태 전이 규칙은 admission-api 한 곳에만 있다 (ADR-0004)
- 전형료·전형명·모집단위가 **카탈로그 API 에서** 온다. 화면에 박혀 있지 않다
